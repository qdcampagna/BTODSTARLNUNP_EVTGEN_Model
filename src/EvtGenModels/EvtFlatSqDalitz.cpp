...
void EvtFlatSqDalitz::decay( EvtParticle* p )
{
    ...
    const double m12 = 0.5 * ( cos( mPrime * EvtConst::pi ) + 1 ) *
    ...
    const double p1 = sqrt( en1 * en1 - mDaug1Sq );
    const double p3 = sqrt( en3 * en3 - mDaug3Sq );
    const double m13Sq =
        mDaug1Sq + mDaug3Sq +
        2.0 * ( en1 * en3 - p1 * p3 * cos( EvtConst::pi * thetaPrime ) );
