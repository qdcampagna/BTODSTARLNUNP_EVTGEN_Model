                                  ...
                                  double& poleSize )
{
  ...
  if(g>0){
    ...
    mH.push_back(std::min( EvtPDL::getMaxMass( meson ), m - mass[1] - mass[2]));
  ...
  for (double m0: mH) {
	...
	Prho = sqrt( ( Erho - m0 ) * ( Erho + m0 ) );
      ...
      double Elep = sqrt( q2 ) * 0.5, Plep = sqrt( Elep * Elep - mass[1] * mass[1] );
  ...
  //  std::cout<<"max "<<maxfoundprob<<" "<<maxpole<<" "<<poleSize<<std::endl;
  maxfoundprob *= 2.9;
  return maxfoundprob;
}
